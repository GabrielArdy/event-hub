...
@Component({
  ...
  styles: [`
    .page { max-width: 480px; margin: 0 auto; padding: 64px 24px; text-align: center; }
  ...
  template: `
    ...
    <div class="page">
      ...
      <p class="desc">Pesanan kamu telah dibatalkan karena waktu pembayaran sudah habis. Kamu bisa membuat pesanan baru.</p>

      <div class="actions">
        @if (eventId()) {
          <a [routerLink]="['/checkout', eventId()]">
            <button pButton type="button" label="Buat Pesanan Baru"
              style="width: 100%; border-radius: 9999px; background: #6C63FF; border-color: #6C63FF;"></button>
          </a>
        }
        <a routerLink="/">
          <button pButton type="button" label="Kembali ke Beranda" outlined
